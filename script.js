...
const supabaseUrl = "https://verlqmqpysxtwzbaybni.supabase.co";
const supabaseKey = "sb_publishable_haL6oXAABaBRH3nuQJV80A_thvybHRn";
  ...
  if (checkoutForm) {
    checkoutForm.addEventListener('submit', async function(e) {
      ...
      const ordersData = activeCartItems.map(item => {
        ...
        const packLabel = item.packageLabel || item.packLabel || item.pack || '';

        return {
          gameTitle: item.gameTitle || item.game || item.title || 'N/A',
          packageLabel: packLabel,
          uid: item.accountUID || item.uid || '',
      ...
      try {
        console.log('Sending orders to API:', { user_id: userId, orders: ordersData });
